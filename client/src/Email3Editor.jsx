...
export default function Email3Editor({
  plantilla,
  userId = 1,
  role: roleProp = 'user',
  ...
  const [ctaAfter4Enabled, setCtaAfter4Enabled] = useState(false)
  const [ctaAfter4ColorIdx, setCtaAfter4ColorIdx] = useState(0)
  const [ctaAfter4Text, setCtaAfter4Text] = useState('')
  const [ctaAfter4Url, setCtaAfter4Url] = useState('')
  const [ctaAfter4FontSizePx, setCtaAfter4FontSizePx] = useState(18)
  const [ctaAfter4Align, setCtaAfter4Align] = useState('center')
  const [ctaAfter5Enabled, setCtaAfter5Enabled] = useState(false)
  const [ctaAfter5ColorIdx, setCtaAfter5ColorIdx] = useState(0)
  const [ctaAfter5Text, setCtaAfter5Text] = useState('')
  const [ctaAfter5Url, setCtaAfter5Url] = useState('')
  const [ctaAfter5FontSizePx, setCtaAfter5FontSizePx] = useState(18)
  const [ctaAfter5Align, setCtaAfter5Align] = useState('center')
  ...
  const showCta4Preview = useMemo(
    () => isEmail1CtaPreviewVisible(ctaAfter4Enabled, ctaAfter4Text, ctaAfter4Url),
    [ctaAfter4Enabled, ctaAfter4Text, ctaAfter4Url]
  )
  const showCta5Preview = useMemo(
    () => isEmail1CtaPreviewVisible(ctaAfter5Enabled, ctaAfter5Text, ctaAfter5Url),
    [ctaAfter5Enabled, ctaAfter5Text, ctaAfter5Url]
  )
  ...
  const cta4Bg = FONDO_CUADRO_IMAGEN_HEX[ctaAfter4ColorIdx] ?? FONDO_CUADRO_IMAGEN_HEX[0]
  const cta5Bg = FONDO_CUADRO_IMAGEN_HEX[ctaAfter5ColorIdx] ?? FONDO_CUADRO_IMAGEN_HEX[0]
  ...
  const cta4Href = sanitizeCtaHttpUrl(ctaAfter4Url)
  const cta5Href = sanitizeCtaHttpUrl(ctaAfter5Url)
  ...
  const applyPayload = useCallback((p) => {
    ...
    if (typeof p.tituloHtml === 'string') setTituloHtml(p.tituloHtml)
    ...
    if (typeof p.cuerpo4Html === 'string') setCuerpo4Html(p.cuerpo4Html)
    if (typeof p.cuerpo5Html === 'string') setCuerpo5Html(p.cuerpo5Html)
    ...
    if (typeof p.ctaAfter4Enabled === 'boolean') setCtaAfter4Enabled(p.ctaAfter4Enabled)
    if (typeof p.ctaAfter4ColorIdx === 'number') setCtaAfter4ColorIdx(p.ctaAfter4ColorIdx)
    if (typeof p.ctaAfter4Text === 'string') setCtaAfter4Text(p.ctaAfter4Text)
    if (typeof p.ctaAfter4Url === 'string') setCtaAfter4Url(p.ctaAfter4Url)
    if (typeof p.ctaAfter4FontSizePx === 'number') setCtaAfter4FontSizePx(p.ctaAfter4FontSizePx)
    if (p.ctaAfter4Align === 'left' || p.ctaAfter4Align === 'center' || p.ctaAfter4Align === 'right') {
      setCtaAfter4Align(p.ctaAfter4Align)
    }
    if (typeof p.ctaAfter5Enabled === 'boolean') setCtaAfter5Enabled(p.ctaAfter5Enabled)
    if (typeof p.ctaAfter5ColorIdx === 'number') setCtaAfter5ColorIdx(p.ctaAfter5ColorIdx)
    if (typeof p.ctaAfter5Text === 'string') setCtaAfter5Text(p.ctaAfter5Text)
    if (typeof p.ctaAfter5Url === 'string') setCtaAfter5Url(p.ctaAfter5Url)
    if (typeof p.ctaAfter5FontSizePx === 'number') setCtaAfter5FontSizePx(p.ctaAfter5FontSizePx)
    if (p.ctaAfter5Align === 'left' || p.ctaAfter5Align === 'center' || p.ctaAfter5Align === 'right') {
      setCtaAfter5Align(p.ctaAfter5Align)
    }
  }, [])
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      try {
        ...
        if (cancelled) return
        if (row.editor_tipo && row.editor_tipo !== 'email3') {
          setApiError('Esta solicitud es de otro editor (no Email 3).')
          return
        }
  ...
  const asideEditorial = (
    <>
      ...
      <div>
        <label htmlFor="email3-asunto-correo" className="mb-1 block text-xs font-medium text-slate-600">
          Asunto del correo
        </label>
        <input
          id="email3-asunto-correo"
          type="text"
          value={asuntoCorreo}
      ...
      />
      <p className="text-[10px] leading-snug text-slate-500">
        En el correo, este botón va en la columna izquierda junto al segundo bloque, antes de la imagen 1.
      </p>
      ...
      <RichTextEmailField
        ...
        colors={TEXTO_MARCA_HEX}
        placeholder="Cuarto bloque (columna izquierda junto a la imagen 2)…"
      />

      <EmailCtaAsideBlock
        enabled={ctaAfter4Enabled}
        onEnabledChange={setCtaAfter4Enabled}
        colorIdx={ctaAfter4ColorIdx}
        onColorIdxChange={setCtaAfter4ColorIdx}
        text={ctaAfter4Text}
        onTextChange={setCtaAfter4Text}
        url={ctaAfter4Url}
        onUrlChange={setCtaAfter4Url}
        fontSizePx={ctaAfter4FontSizePx}
        onFontSizeChange={setCtaAfter4FontSizePx}
        align={ctaAfter4Align}
        onAlignChange={setCtaAfter4Align}
      />
      <p className="text-[10px] leading-snug text-slate-500">
        En el correo, este botón va en la columna izquierda (70%) junto al cuarto bloque, antes de la imagen 2.
      </p>
      ...
      <div>
        ...
        <div className="mb-2 rounded-lg border border-slate-200 bg-white p-2">
          <div
            className="mx-auto h-28 w-20 max-w-full rounded-xl border border-white/60 shadow-sm"
            style={{ backgroundColor: bgImg2, aspectRatio: '5 / 7' }}
          />
        </div>
        <div className="flex flex-wrap gap-2">
          {FONDO_CUADRO_IMAGEN_HEX.map((hex, i) => (
      ...
      <RichTextEmailField
        label="Quinto bloque de texto"
        value={cuerpo5Html}
        onChange={(v) => setCuerpo5Html(v)}
        colors={TEXTO_MARCA_HEX}
        placeholder="Quinto bloque de texto…"
      />

      <EmailCtaAsideBlock
        enabled={ctaAfter5Enabled}
        onEnabledChange={setCtaAfter5Enabled}
        colorIdx={ctaAfter5ColorIdx}
        onColorIdxChange={setCtaAfter5ColorIdx}
        text={ctaAfter5Text}
        onTextChange={setCtaAfter5Text}
        url={ctaAfter5Url}
        onUrlChange={setCtaAfter5Url}
        fontSizePx={ctaAfter5FontSizePx}
        onFontSizeChange={setCtaAfter5FontSizePx}
        align={ctaAfter5Align}
        onAlignChange={setCtaAfter5Align}
      />
      <p className="text-[10px] leading-snug text-slate-500">
        En el correo, este botón va justo después del quinto bloque de texto.
      </p>
      ...
      <div className="flex min-h-full w-full min-w-0 items-center justify-center p-6">
        ...
        >
          ...
          <div className="space-y-4 px-6 py-5">
            ...
            <div className="w-full max-w-full min-w-0" style={gridRowTexto4Img2}>
              <div className={`min-w-0 max-w-full space-y-3 ${richClass}`} style={richStyle}>
                {show4 ? <div dangerouslySetInnerHTML={{ __html: preview4 }} /> : null}
                {showCta4Preview && cta4Href ? (
                  <div className={`flex w-full ${ctaPreviewJustify(ctaAfter4Align)}`}>
                    <a
                      href={cta4Href}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="inline-block rounded-[30px] px-5 py-3.5 text-center no-underline"
                      style={{
                        backgroundColor: cta4Bg,
                        color: '#ffffff',
                        fontFamily: "'Nunito Sans', Verdana, Geneva, sans-serif",
                        fontWeight: 900,
                        fontSize: clampCtaFontSizePx(ctaAfter4FontSizePx),
                        lineHeight: 1.25,
                      }}
                    >
                      {ctaAfter4Text.trim()}
            ...
            {showCta5Preview && cta5Href ? (
              <div className={`mt-2 flex w-full pb-2 ${ctaPreviewJustify(ctaAfter5Align)}`}>
                <a
                  href={cta5Href}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-block rounded-[30px] px-5 py-3.5 text-center no-underline"
                  style={{
                    backgroundColor: cta5Bg,
                    color: '#ffffff',
                    fontFamily: "'Nunito Sans', Verdana, Geneva, sans-serif",
                    fontWeight: 900,
                    fontSize: clampCtaFontSizePx(ctaAfter5FontSizePx),
                    lineHeight: 1.25,
                  }}
                >
                  {ctaAfter5Text.trim()}
