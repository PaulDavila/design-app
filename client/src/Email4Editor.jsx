...
function EmailNanoBananaPromptField({ label, value, onChange, idSuffix }) {
  const fieldId = `email4-nb-${idSuffix}`
  return (
    <div>
...
export default function Email4Editor({
  plantilla,
  userId = 1,
  role: roleProp = 'user',
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      try {
        ...
        if (cancelled) return
        if (row.editor_tipo && row.editor_tipo !== 'email4') {
          setApiError('Esta solicitud es de otro editor (no Email 4).')
          return
        }
  ...
  const asideEditorial = (
    <>
      <div>
        <h2 className="text-sm font-semibold text-slate-900">Email 4</h2>
        {identityFromUrl ? (
      ...
      <div>
        <label htmlFor="email4-asunto-correo" className="mb-1 block text-xs font-medium text-slate-600">
          Asunto del correo
        </label>
        <input
          id="email4-asunto-correo"
          type="text"
          value={asuntoCorreo}
      ...
      <RichTextEmailField
        ...
        colors={TEXTO_MARCA_HEX}
        placeholder="Texto debajo de las imágenes…"
      />
      ...
      <div className="flex min-h-full w-full min-w-0 items-center justify-center p-6">
        ...
        >
          ...
          <div className="space-y-4 px-6 py-5">
            ...
            <div
              className={`flex gap-3 ${isMobilePreview ? 'flex-col' : 'flex-row items-stretch'}`}
            >
              <div
                className={`relative min-w-0 overflow-hidden rounded-2xl ${
                  isMobilePreview ? 'aspect-square w-full' : 'w-1/3 shrink-0'
                }`}
                style={{ backgroundColor: bgImg1, aspectRatio: '1 / 1' }}
              >
                {imagenGemini1Url ? (
              ...
              <div
                className={`relative min-w-0 overflow-hidden rounded-2xl ${
                  isMobilePreview ? 'aspect-video w-full' : 'min-h-0 flex-1 self-stretch'
                }`}
                style={{ backgroundColor: bgImg2, minHeight: isMobilePreview ? undefined : 120 }}
              >
                {imagenGemini2Url ? (
                  <div className="absolute inset-0 overflow-hidden rounded-2xl">
                    <img
                  ...
                  <div className="absolute inset-0 flex min-h-[120px] items-center justify-center text-center text-[11px] font-medium text-slate-700/70">
                    Ilustración Gemini PNG
                  </div>
                )}
              </div>
            ...
            {show2 && (
              <div className={richClass} style={richStyle} dangerouslySetInnerHTML={{ __html: preview2 }} />
            )}

            {showCta2Preview && cta2Href ? (
              <div className={`mt-2 flex w-full pb-2 ${ctaPreviewJustify(ctaAfter2Align)}`}>
                <a
                  href={cta2Href}
                  target="_blank"
                  rel="noopener noreferrer"
