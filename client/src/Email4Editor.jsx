...
export default function Email4Editor({ plantilla }) {
  ...
  const exportarPdf = async () => {
    ...
    try {
      ...
      pdf.save(`email-4-${plantilla?.id_externo || 'preview'}.pdf`)
    } finally {
      setExporting(false)
    }
  }
  ...
  return (
    <div className="flex min-h-[calc(100vh-5rem)] flex-col gap-4 lg:flex-row">
      <aside className="w-full shrink-0 space-y-4 overflow-y-auto rounded-2xl border border-slate-200 bg-white p-4 shadow-sm lg:sticky lg:top-24 lg:max-h-[calc(100vh-6rem)] lg:w-[380px] lg:self-start">
        ...
        <div>
          <label className="mb-1 block text-xs font-medium text-slate-600">
            Instrucciones para la IA (imagen)
          </label>
          <textarea
            id="email4-prompt-ia"
            value={promptIA}
      ...
      <section className="flex min-w-0 flex-1 flex-col rounded-2xl border border-slate-200 bg-slate-50/80 p-4 shadow-sm">
          ...
          >
            ...
            >
              ...
              <div className="space-y-4 px-6 py-5">
                ...
                <div className={`grid gap-3 ${vistaPreview === 'mobile' ? 'grid-cols-1' : 'grid-cols-3'}`}>
                  <div
                    className="relative col-span-1 overflow-hidden rounded-2xl"
                    style={{ backgroundColor: bgImg1, minHeight: 170 }}
                  >
                  ...
                  <div
                    ...
                    style={{ backgroundColor: bgImg2, minHeight: 170 }}
                  >
                    <div className="absolute inset-0 flex items-center justify-center text-center text-[11px] font-medium text-slate-700/70">
                      Ilustración Gemini PNG
                    </div>
        ...
        {promptIA.trim() && (
          <p className="mt-2 text-xs text-amber-800/90">
            <strong>IA (borrador):</strong> {promptIA.slice(0, 200)}
            {promptIA.length > 200 ? '…' : ''}
          </p>
        )}
      </section>
