...
/** Mismo bloque que Email 1: instrucciones IA (imagen), sin conexión aún. */
...
export default function Email2Editor({ plantilla }) {
  ...
  /**
   * Fila fija: texto 2 e imagen 1 siempre comparten fila en escritorio (60% / 40%, gap 10px).
   * Antes: si el segundo bloque estaba vacío, show2 era false y el grid pasaba a 1 columna → la
   * imagen 1 ocupaba todo el ancho y parecía “mal”.
   */
  ...
  const logoRuta = useMemo(() => {
    try {
      ...
      return (
        d?.email2?.logoRuta || d?.email1?.logoRuta || 'miniaturas/logo-abc-logistica.svg'
      )
    } catch {
  ...
  const exportarPdf = async () => {
    ...
    try {
      ...
      pdf.save(`email-2-${plantilla?.id_externo || 'preview'}.pdf`)
    } finally {
      setExporting(false)
    }
  }
