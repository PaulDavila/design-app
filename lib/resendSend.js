const { MAIL_DISPLAY_NAME } = require('./mailFrom');

/** Mismo remitente de prueba que el snippet del dashboard de Resend (dominio verificado por ellos). */
const RESEND_FROM_DEFAULT = `${MAIL_DISPLAY_NAME} <onboarding@resend.dev>`;
...
async function sendViaResend(opts) {
  ...
  if (!from) {
    // No usar buildFromHeader() aquí: suele ser Gmail/SMTP_USER y Resend lo rechaza si el dominio no está verificado.
